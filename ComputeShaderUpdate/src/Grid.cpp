...
Grid::Grid(uint32_t _w, uint32_t _h,size_t _numParticles,int _groupSize) 
{
    m_width=_w;
    m_height=_h;
    m_groupSize=_groupSize;

    m_numParticles=roundTo(_numParticles,m_groupSize);
    initGrid();
...
void Grid::update(float _dt)
{
  ...
  glDispatchCompute(m_numParticles/m_groupSize, 1, 1);
...
/*
  ...
  {
    m_posAccel[i].m_x=0.3f; //ngl::Random::randomNumber(m_width/2.0f);
    m_posAccel[i].m_z=0.3f; //ngl::Random::randomNumber(m_height/2.0f);
    m_posAccel[i].m_y=0.0f; // just in case!
    m_posAccel[i].m_w= 15.3f;//ngl::Random::randomPositiveNumber(5) + 0.5f;
    m_dirSpeed[i] = ngl::Random::getRandomVec3()*2.0f;  
    m_dirSpeed[i].m_y=0.0f; // this needs to be done as reflect is 3d
    m_dirSpeed[i].m_w=ngl::Random::randomPositiveNumber(5)+0.5f;  
  }

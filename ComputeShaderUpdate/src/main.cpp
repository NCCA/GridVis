...
int main(int argc, char **argv)
{
  ...
  uint32_t width=parser.value("width").toInt();
  uint32_t height=parser.value("height").toInt();
  uint32_t numParticles=parser.value("numParticles").toInt();
  std::cout<<width<<' '<<height<<' '<<numParticles;
  NGLScene window(width,height,numParticles);
